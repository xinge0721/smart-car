...
// 接入gnss TAU1201连接
//      模块管脚            单片机管脚
//      RX                  查看 zf_device_gnss.h 文件中 gnss_RX 宏定义的引脚 
//      TX                  查看 zf_device_gnss.h 文件中 gnss_TX 宏定义的引脚 
//      GND                 电源地
//      VCC                 5V电源
// 接入IPS2.0屏幕
//      模块管脚            单片机管脚
...
int main(void)
{
    ...
    while(true)
    {
        ...
        if(gnss_flag)
        {
            ...
            ips200_show_float(120, 16*3, (double)gnss.speed, 4, 6);
            ips200_show_float(  0, 16*4, (double)gnss.direction, 4, 6);
            ips200_show_uint( 120, 16*4, gnss.satellite_used, 5);
            ips200_show_float(  0, 16*5, (double)gnss.height, 4, 6);
        }
        else
        {
            no_data_count ++;
